...
function drain(source: MeetingRecord.AudioTrack, writer: WavWriter, into: () => Recording | null): Track {
...
ipcMain.handle('permission:status', (_event, name: PermissionName) => MeetingRecord.permissions.status(name))
ipcMain.handle('permission:request', (_event, name: PermissionName) => MeetingRecord.permissions.request(name))
