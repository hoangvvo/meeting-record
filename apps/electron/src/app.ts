...
const PERMISSION_TEXT: Record<
  PermissionStatus,
  keyof Pick<PermissionUi, "ask" | "granted" | "denied">
> = {
  unknown: "ask",
  granted: "granted",
  denied: "denied",
...
function onMeetingEvent({ kind, meeting }: MeetingEvent): void {
  ...
  const recordingHint = meeting.shouldRecord ? " (should record)" : "";
  log(
    `${kind} ${label(meeting)}: ${meeting.appName}, confidence ${meeting.confidence}${recordingHint}`,
  );
}
...
async function stop(): Promise<void> {
  ...
  try {
    ...
    log(
      `wrote ${result.path} (${result.seconds.toFixed(1)}s, peak ${result.peak.toFixed(3)}${dropped})`,
    );
